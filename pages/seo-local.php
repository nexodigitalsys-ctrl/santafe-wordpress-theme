    ...
    <div class="max-w-7xl mx-auto px-6">
        <div class="max-w-4xl">
            ...
            <div class="flex flex-wrap gap-4 mt-10">
                <a href="/<?php echo $lang; ?>/contacto/" class="inline-flex items-center bg-brand-600 hover:bg-brand-500 text-warm-900 font-semibold px-8 py-4 rounded-sm transition-all tracking-wide text-sm uppercase"><?php echo $cta_budget; ?></a>
...
<section class="py-20 bg-warm-50 border-y border-warm-200">
    <div class="max-w-7xl mx-auto px-6 grid lg:grid-cols-3 gap-6">
        <article class="bg-white border border-warm-200 p-7 rounded-sm">
            <h2 class="font-display font-bold text-2xl text-warm-900 mb-3"><?php echo $price_title; ?></h2>
            <p class="text-warm-500"><?php echo $is_ca ? 'Referència inicial: ' : 'Referencia inicial: '; ?><?php echo esc_html($data['rate']); ?>. <?php echo $is_ca ? 'La xifra final depèn de mesuraments, qualitats, llicències i estat inicial.' : 'La cifra final depende de mediciones, calidades, licencias y estado inicial.'; ?></p>
        </article>
        <article class="bg-white border border-warm-200 p-7 rounded-sm">
            <h2 class="font-display font-bold text-2xl text-warm-900 mb-3"><?php echo $process_title; ?></h2>
            <p class="text-warm-500"><?php echo $is_ca ? 'Primer visita i abast. Després partides, calendari i fases. Durant l\'obra, decisions documentades.' : 'Primero visita y alcance. Después partidas, calendario y fases. Durante la obra, decisiones documentadas.'; ?></p>
        </article>
        <article class="bg-white border border-warm-200 p-7 rounded-sm">
            <h2 class="font-display font-bold text-2xl text-warm-900 mb-3"><?php echo $zone_title; ?></h2>
            <p class="text-warm-500"><?php echo $is_ca ? 'Traballem a ' : 'Trabajamos en '; ?><?php echo esc_html($data['city']); ?><?php echo $is_ca ? ' i voltants, a més de Barcelona, Girona, Tarragona i altres zones de Catalunya.' : ' y alrededores, además de Barcelona, Girona, Tarragona y otras zonas de Cataluña.'; ?></p>
        </article>
    </div>
</section>
...
<section class="py-24 bg-white">
    <div class="max-w-4xl mx-auto px-6">
        <h2 class="font-display font-bold text-3xl md:text-4xl text-warm-900 tracking-tight mb-10"><?php echo $faq_title; ?></h2>
        <div class="space-y-4">
            <?php foreach ($faqs as $faq): ?>
            <details class="bg-warm-50 border border-warm-200 rounded-sm p-6">
                <summary class="cursor-pointer font-semibold text-warm-900"><?php echo esc_html($faq['q']); ?></summary>
                <p class="text-warm-500 mt-4 leading-relaxed"><?php echo esc_html($faq['a']); ?></p>
            </details>
            <?php endforeach; ?>
        </div>
    </div>
